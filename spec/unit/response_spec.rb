...
describe RestClient::Response, :include_helpers do
  ...
  describe "header processing" do
    ...
    it "handles multiple headers by joining with comma" do
      net_http_res = res_double(to_hash: { "My-Header" => %w{foo bar} }, code: "200", body: nil)
      example_url = "http://example.com"
      request = request_double(url: example_url, method: "get")
      response = response_from_res_double(net_http_res, request)

      expect(response.raw_headers["My-Header"]).to eq %w{foo bar}
  ...
  describe "redirection" do
    ...
    it "follows a redirection and keep the parameters" do
      stub_request(:get, "http://some/resource").with(headers: { "Accept" => "application/json" }, basic_auth: %w{foo bar}).to_return(body: "", status: 301, headers: { "Location" => "http://new/resource" })
      stub_request(:get, "http://new/resource").with(headers: { "Accept" => "application/json" }, basic_auth: %w{foo bar}).to_return(body: "Foo")
